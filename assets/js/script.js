...
var getAnime = {
 ...
 episode: async function(eps_slug,
  ...
  if (eps.status != "Ok") {
   ...
   document.title = `${eps.data.episode} - WanRzy Nime`;
   document.getElementsByTagName("meta")["description"].content = `Unduh, Nonton, & Streaming Anime ${eps.data.episode} resolusi 360p, 480p, 720p, HD, HR format Mp4 dan Mkv lengkap beserta Batch. (Situs ini dibuat dengan API/Scraper)`;
   document.querySelector("meta[property='og:title']").content = `${eps.data.episode} - WanRzy Nime`;
   document.querySelector("meta[property='og:description']").content = `Unduh, Nonton, & Streaming Anime ${eps.data.episode} resolusi 360p, 480p, 720p, HD, HR format Mp4 dan Mkv lengkap beserta Batch. (Situs ini dibuat dengan API/Scraper)`;
 ...
 info: function(slug) {
  fetch("https://otakudesu-unofficial-api.vercel.app/v1/anime/" + slug).then((r) => r.json()).then((j) => {
   // console.log(j);
   if (j.status == "Ok") {
    document.title = `${j.data.title} - WanRzy Nime`;
    ...
    document.querySelector("meta[property='og:title']").content = `${j.data.title} - WanRzy Nime`;
    document.querySelector("meta[property='og:description']").content = `Unduh, Nonton, & Streaming Anime ${j.data.title} resolusi 360p, 480p, 720p, HD, HR format Mp4 dan Mkv lengkap beserta Batch. (Situs ini dibuat dengan API/Scraper)`;
